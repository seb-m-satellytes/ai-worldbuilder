...
def generate_regions(country_size: int, country_population: int):
    ...
    no_of_new_regions = max(
        1, min(no_of_new_regions, country_population // min_population_per_region))

    # Generate Gaussian distributed population values
    mean_population = country_population / no_of_new_regions
    # Arbitrary standard deviation for more variability
    std_dev_population = mean_population / 3
    populations = np.random.normal(
        mean_population, std_dev_population, no_of_new_regions)
