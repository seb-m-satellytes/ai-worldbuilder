...
def distribute_population_in_category(base_population, stddev, num_units):
    ...
    while any(population_values <= 0):
        population_values = np.where(population_values <= 0, np.random.normal(
            base_population, stddev, 1), population_values)
...
def distribute_metropolises(total_population, regional_type):
    metropolitan_population = calculate_population(
        total_population, "metropolis", regional_type)
    metropolis_base_population = 1_500_000
    no_of_new_metropolises = round(
        metropolitan_population / metropolis_base_population)

    metropolises = []
    metropolitan_population_absolute = 0

    if no_of_new_metropolises > 0:
        new_metropolises = distribute_population_in_category(
            metropolis_base_population, 350_000, no_of_new_metropolises)
...
def distribute_cities(total_population, regional_type):
    cityish_population = calculate_population(
        total_population, "city", regional_type)
    ...
    if no_of_new_cities > 0:
        new_cities = distribute_population_in_category(
            cityish_base_population, 50_000, no_of_new_cities)
...
def distribute_towns(total_population, regional_type):
    townish_population = calculate_population(
        total_population, "town", regional_type)
    ...
    if no_of_new_towns > 0:
        new_towns = distribute_population_in_category(
            town_base_population, 5_000, no_of_new_towns)
...
def distribute_villages(total_population):
    ...
    if no_of_new_villages > 0:
        new_villages = distribute_population_in_category(
            village_base_population, 850, no_of_new_villages)
...
def distribute_population(total_population: int):
    ...
    metropolises, metropolitan_population_absolute = distribute_metropolises(
        total_population, regional_type)
    left_to_distribute = total_population - metropolitan_population_absolute

    cities, cityish_population_absolute = distribute_cities(
        left_to_distribute, regional_type)
    left_to_distribute -= cityish_population_absolute

    towns, townish_population_absolute = distribute_towns(
        left_to_distribute, regional_type)
    left_to_distribute -= townish_population_absolute

    villages, village_population_absolute = distribute_villages(
        left_to_distribute)

    met_perc = print_distribution_info(
        "metropolises", metropolises, total_population, metropolitan_population_absolute)
    cit_perc = print_distribution_info(
        "cities", cities, total_population, cityish_population_absolute)
    town_perc = print_distribution_info(
        "towns", towns, total_population, townish_population_absolute)
    vil_perc = print_distribution_info(
        "villages", villages, total_population, village_population_absolute)
